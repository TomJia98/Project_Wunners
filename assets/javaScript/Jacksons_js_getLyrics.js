...
function startTimer() {
  // adds the timer, which stops if roundFinished is set to True
  var timeInterval = setInterval(function () {
    if (roundFinished) {
      //stops the timer if the game is over, mainly to avoid errors
...
function settingUl(inputScore) {
  //appends the scores currently in LocalStorage to the page
  const li = $("<li>");
  li.text(
    "Name: " +
      inputScore.name +
  ...
  $.ajax({
    ...
    $.ajax({
      ...
      function changeLyrics() {
        //self made for loop with different outcomes based on the fetch response
        if (isFirstRun) {
          // this cannot be a regular for loop because of the asyncryonous property of the fetch method
          if (loop < lyricsArr.length) {
              ...
              fetch(
                "https://wordsapiv1.p.rapidapi.com/words/" + word + "/typeOf",
                {
                  ...
                  headers: {
                    ...
                    "x-rapidapi-key":
                      "89589060b5mshffab6adbe6bfb7ap1a6f99jsnddb2235d4e56",
                  },
                }
              )
                .then(function (resp) {
                ...
                .then(function (data1) {
                  if (data1.typeOf.length == 0) {
                    console.log(
                      lyricsArr[loop] + " --- " + "no similar words, skipping"
                    );
                    loop++;
                    changeLyrics();
                    ...
                    if (
                      lyricsArr[loop].charAt(0).toUpperCase() !=
                      lyricsArr[loop].charAt(0)
                    ) {
                      console.log(lyricsArr[loop]);
                      lyricsArr[loop] =
                        data1.typeOf[getRandomInt(data1.typeOf.length)];
                ...
                .catch(function () {
                  console.log(
                    lyricsArr[loop] + " --- " + "word not found, skipping"
                  );
                  loop++;
                  changeLyrics();
            ...
            for (var i = 0; i < lyricsArr.length - 1; i++) {
              ...
              if (
                (x.toUpperCase() == x && lyricsArr[i + 1].length != 1) ||
                x == undefined
              ) {
                var liEl = $("<li>");
                liEl.text(WIP);
            ...
            isFirstRun = false;
          } //only allows the lyrics to be run once (unless isFirstRun is redefined by a clear)
        }
      }
      changeLyrics();
    });
...
$("#submit-answer").on("click", function clickSubmit() {
  if (answer.val() == correctSong) {
    ...
    setTimeout(() => {
      $("#incorrect-guess").attr("style", "display:none;");
    }, 1000);
...
saveScoreBtn.on("click", function saveHighScore(e) {
  ...
  if (newScore.name == "") {
    ...
    setTimeout(() => {
      $("#no-username").attr("style", "display:none;");
    }, 1000);
  } // if the answer is incorrect, add 10 seconds to the timer and tell the user they where incorrect
  else {
...
songsAmount.on("input", function () {
  //changes the value on the first page to show the user the sliders value
...
tryAgainBtn.on("click", function resetPage() {
  //the click event for the try again button on page 3
  location.reload();
}); //reloads the page to reset all the variables back to their beginning state
// ___________________________________________________________________________

clearHighscores.on("click", function () {
  //the click event for the clear highscores button on page 3
  localStorage.clear();
  location.reload();
  firstPage.attr("style", "display:none;");
  thirdPage.attr("style", "display:;");
...
checkHighscores.on("click", function () {
  // the click event for the check highscores button on page 1
  firstPage.attr("style", "display:none;");
  thirdPage.attr("style", "display:;");
  username.attr("style", "display: none;");
  saveScoreBtn.attr("style", "display:none;");
